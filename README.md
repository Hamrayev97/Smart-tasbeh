...
- **Navigation:** bottom tabs (Counter, Statistics, Dhikr List, Qibla, Ramadan, Azkar, Settings)
...
- **Azkar library:** `src/data/azkar.json` — 34 morning/evening adhkar (Arabic text, transliteration, English translation, repetition count), split into Morning/Evening tabs. Content is the English dataset from [Seen-Arabic/Morning-And-Evening-Adhkar-DB](https://github.com/Seen-Arabic/Morning-And-Evening-Adhkar-DB) (MIT license, sourced from Hisnul Muslim), trimmed to the fields the app displays. The Arabic/English text itself isn't re-translated per app language — only the screen's UI labels are.
