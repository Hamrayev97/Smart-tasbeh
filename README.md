...
- **Monetization:** free/premium state; AdMob banner ads on the Counter and Statistics screens (hidden for premium users), dimmed to ~65% opacity in dark mode so they don't clash with the dark UI; interstitial placement is still a placeholder. A "Remove Ads" one-time purchase (`react-native-iap`) sets `premium` for real on Android/iOS. A donation button opens an external link (Android/iOS/web).
...
- `src/components/RemoveAdsButton.js` uses a placeholder product ID (`remove_ads`, see `REMOVE_ADS_SKU`). Create a matching non-consumable in-app product in Play Console / App Store Connect and update the SKU before publishing. Purchases only work on Android/iOS (`react-native-iap` has no web implementation), and can't be tested without a real store listing.
